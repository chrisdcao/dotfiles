" Name:         WWDC16
" Description:  Colorful dark color scheme for Vim inspired by Apple's WWDC16 page
" Author:       Lifepillar <lifepillar@lifepillar.me>
" Maintainer:   Lifepillar <lifepillar@lifepillar.me>
" Website:      https://github.com/lifepillar/vim-wwdc16-theme
" License:      MIT
" Last Updated: Sun Nov 15 22:39:51 2020

" Generated by Colortemplate v2.1.0

set background=dark

hi clear
let g:colors_name = 'wwdc16'

let s:t_Co = exists('&t_Co') && !empty(&t_Co) && &t_Co > 1 ? &t_Co : 2
let s:italics = (&t_ZH != '' && &t_ZH != '[7m') || has('gui_running') || has('nvim')

hi! link Repeat Repeat
hi! link QuickFixLine Search
hi! link StatusLineTerm StatusLine
hi! link StatusLineTermNC StatusLineNC
hi! link Define PreProc
hi! link Float Number
hi! link Exception Statement
hi! link Macro PreProc
hi! link PreCondit PreProc
hi! link Tag Special
hi! link lCursor Cursor

hi! link javaStorageClass Structure
hi! link javaStructure StorageClass

if (has('termguicolors') && &termguicolors) || has('gui_running')
  let g:terminal_ansi_colors = ['#292c36', '#e64547', '#42a647', '#c98351',
        \ '#4670d8', '#d24e9b', '#64878f', '#949494', '#353547', '#f84547',
        \ '#95c76f', '#efa16b', '#5283ff', '#8485ce', '#00aba5', '#e7e8eb']
  if has('nvim')
    let g:terminal_color_0 = '#292c36'
    let g:terminal_color_1 = '#e64547'
    let g:terminal_color_2 = '#42a647'
    let g:terminal_color_3 = '#c98351'
    let g:terminal_color_4 = '#4670d8'
    let g:terminal_color_5 = '#d24e9b'
    let g:terminal_color_6 = '#64878f'
    let g:terminal_color_7 = '#949494'
    let g:terminal_color_8 = '#353547'
    let g:terminal_color_9 = '#f84547'
    let g:terminal_color_10 = '#95c76f'
    let g:terminal_color_11 = '#efa16b'
    let g:terminal_color_12 = '#5283ff'
    let g:terminal_color_13 = '#8485ce'
    let g:terminal_color_14 = '#00aba5'
    let g:terminal_color_15 = '#e7e8eb'
  endif
  if get(g:, 'wwdc16_transp_bg', 0) && !has('gui_running')
    "292c36 - lower-constrast background"
    "20222b - highr-constrast background"
    hi Normal guifg=#e7e8eb guibg=NONE gui=NONE cterm=NONE
    hi CursorLineNr guifg=#e7e8eb guibg=NONE gui=NONE cterm=NONE
    hi FoldColumn guifg=#949494 guibg=NONE gui=NONE cterm=NONE
    hi LineNr guifg=#949494 guibg=NONE gui=NONE cterm=NONE
    hi Terminal guifg=#e7e8eb guibg=NONE gui=NONE cterm=NONE
  else
    hi Normal guifg=#e7e8eb guibg=#292c36 gui=NONE cterm=NONE
    hi CursorLineNr guifg=#e7e8eb guibg=#292c36 gui=NONE cterm=NONE
    hi FoldColumn guifg=#949494 guibg=#353547 gui=NONE cterm=NONE
    "hi LineNr guifg=#595e6d guibg=NONE gui=NONE cterm=NONE
    hi LineNr guifg=#949494 guibg=#3f424a gui=NONE cterm=NONE
    hi Terminal guifg=#e7e8eb guibg=#292c36 gui=NONE cterm=NONE
  endif
  hi ColorColumn guifg=NONE guibg=#353547 gui=NONE cterm=NONE
  hi Conceal guifg=#64878f guibg=NONE gui=NONE cterm=NONE

  hi Cursor guifg=#e7e8eb guibg=#4670d8 gui=NONE cterm=NONE
  hi CursorColumn guifg=NONE guibg=#353547 gui=NONE cterm=NONE
  hi CursorLine guifg=NONE guibg=#404041 gui=NONE cterm=NONE

  hi DiffAdd guifg=#95c76f guibg=#292c36 gui=reverse cterm=reverse
  hi DiffChange guifg=#c98351 guibg=#e7e8eb gui=reverse cterm=reverse
  hi DiffDelete guifg=#e64547 guibg=#e7e8eb gui=reverse cterm=reverse
  hi DiffText guifg=#00aba4 guibg=#e7e8eb gui=bold,reverse cterm=bold,reverse
  hi Directory guifg=#95c76f guibg=NONE gui=NONE cterm=NONE
  hi EndOfBuffer guifg=#353547 guibg=NONE gui=NONE cterm=NONE
  hi ErrorMsg guifg=#e64547 guibg=#e7e8eb gui=reverse cterm=reverse
  hi Folded guifg=#949494 guibg=#353547 gui=italic cterm=italic
  hi MatchParen guifg=#292c36 guibg=#fef937 gui=NONE cterm=NONE
  hi IncSearch guifg=#292c36 guibg=#fef937 gui=NONE cterm=NONE
  hi Search guifg=#e7e8eb guibg=#6b6c71 gui=NONE cterm=NONE
  "hi Search guifg=#c98351 guibg=#e7e8eb gui=reverse cterm=reverse
  "hi IncSearch guifg=#d24e9b guibg=#e7e8eb gui=standout cterm=reverse
  "hi MatchParen guifg=#353547 guibg=#c98351 gui=bold,reverse cterm=bold,reverse
  hi ModeMsg guifg=#e7e8eb guibg=NONE gui=NONE cterm=NONE
  hi MoreMsg guifg=#949494 guibg=NONE gui=NONE cterm=NONE
  hi NonText guifg=#e7e8eb guibg=NONE gui=NONE cterm=NONE
  hi Pmenu guifg=#292c36 guibg=#64878f gui=NONE cterm=NONE
  hi PmenuSbar guifg=#353547 guibg=#353547 gui=NONE cterm=NONE
  hi PmenuSel guifg=#e7e8eb guibg=#c98351 gui=NONE cterm=NONE
  hi PmenuThumb guifg=#353547 guibg=#c98351 gui=NONE cterm=NONE
  hi Question guifg=#949494 guibg=NONE gui=NONE cterm=NONE
  hi SignColumn guifg=#949494 guibg=NONE gui=NONE cterm=NONE
  hi SpecialKey guifg=#949494 guibg=NONE gui=NONE cterm=NONE
  hi SpellBad guifg=#8485ce guibg=NONE guisp=#d24e9b gui=undercurl cterm=underline
  hi SpellCap guifg=#8485ce guibg=NONE guisp=#d24e9b gui=undercurl cterm=underline
  hi SpellLocal guifg=#8485ce guibg=NONE guisp=#d24e9b gui=undercurl cterm=underline
  hi SpellRare guifg=#8485ce guibg=NONE guisp=#d24e9b gui=undercurl cterm=underline
  hi StatusLine guifg=#64878f guibg=#e7e8eb gui=reverse cterm=reverse
  hi StatusLineNC guifg=#64878f guibg=#292c36 gui=reverse cterm=reverse
  hi TabLine guifg=#292c36 guibg=#64878f gui=NONE cterm=NONE
  hi TabLineFill guifg=#e7e8eb guibg=#64878f gui=NONE cterm=NONE
  hi TabLineSel guifg=#e7e8eb guibg=#64878f gui=NONE cterm=NONE
  hi Title guifg=#c98351 guibg=NONE gui=bold cterm=bold
  "hi VertSplit guifg=#353547 guibg=#64878f gui=NONE cterm=NONE
  hi VertSplit guifg=#383b45 guibg=#383b45 gui=NONE cterm=NONE
  hi Visual guifg=#4670d8 guibg=#e7e8eb gui=reverse cterm=reverse
  hi VisualNOS guifg=#e7e8eb guibg=#4670d8 gui=NONE cterm=NONE
  hi WarningMsg guifg=#e64547 guibg=NONE gui=NONE cterm=NONE
  hi WildMenu guifg=#e7e8eb guibg=#d24e9b gui=NONE cterm=NONE
  hi Debug guifg=#d24e9b guibg=NONE gui=NONE cterm=NONE
  hi Delimiter guifg=#e7e8eb guibg=NONE gui=NONE cterm=NONE
  hi Error guifg=#e64547 guibg=#e7e8eb gui=reverse cterm=reverse
  hi Function guifg=#18aaa2 guibg=NONE gui=NONE cterm=NONE
  hi Identifier guifg=#00aba5 guibg=NONE gui=NONE cterm=NONE
  hi Ignore guifg=#e7e8eb guibg=NONE gui=NONE cterm=NONE
  hi Label guifg=#8485ce guibg=NONE gui=NONE cterm=NONE

  "hi Type guifg=#64dcd4 guibg=NONE gui=NONE cterm=NONE
  hi Type guifg=#00aba4 guibg=NONE gui=NONE cterm=NONE
  hi Typedef guifg=#95c76f guibg=NONE gui=NONE cterm=NONE
  "hi Type guifg=#b53b94 guibg=NONE gui=NONE cterm=NONE
  hi String guifg=#dc3a3c guibg=NONE gui=NONE cterm=NONE
  hi Character guifg=#c98351 guibg=NONE gui=NONE cterm=NONE
  hi Keyword guifg=#bc25a4 guibg=NONE gui=NONE cterm=NONE
  hi Boolean guifg=#64878f guibg=NONE gui=NONE cterm=NONE
  hi Number guifg=#d28e5d guibg=NONE gui=NONE cterm=NONE
  hi Constant guifg=#64878f guibg=NONE gui=NONE cterm=NONE
  "hi Constant guifg=#d28e5d guibg=NONE gui=NONE cterm=NONE
  hi Operator guifg=#00aba5 guibg=NONE gui=NONE cterm=NONE

  hi PreProc guifg=#d28e5d guibg=NONE gui=NONE cterm=NONE
  hi Include guifg=#d28e5d guibg=NONE gui=NONE cterm=NONE
  "hi Include guifg=#95c76f guibg=NONE gui=NONE cterm=NONE
  hi Statement guifg=#b73999 guibg=NONE gui=NONE cterm=NONE

  hi Repeat guifg=#b53b94 guibg=NONE gui=NONE cterm=NONE
  hi Conditional guifg=#b53b94 guibg=NONE gui=NONE cterm=NONE

  "hi StorageClass guifg=#95c76f guibg=NONE gui=NONE cterm=NONE
  "hi StorageClass guifg=#b9b5f6 guibg=NONE gui=NONE cterm=NONE
  "hi StorageClass guifg=#00aba4 guibg=NONE gui=NONE cterm=NONE
  "hi StorageClass guifg=#64dcd4 guibg=NONE gui=NONE cterm=NONE
  hi StorageClass guifg=#4670d8 guibg=NONE gui=NONE cterm=NONE
  hi Structure guifg=#95c76f guibg=NONE gui=NONE cterm=NONE

  hi Special guifg=#d28e5d guibg=NONE gui=NONE cterm=NONE
  hi SpecialChar guifg=#d24e9b guibg=NONE gui=NONE cterm=NONE
  "derived version"
  "hi Comment guifg=#23ad68 guibg=NONE gui=NONE cterm=NONE
  "original in the wwdc16 conference"
  "more prominent
  "less prominent
  "hi Comment guifg=#57a263 guibg=NONE gui=NONE cterm=NONE
  hi Comment guifg=#52bd58 guibg=NONE gui=NONE cterm=NONE 
  hi SpecialComment guifg=#35d585 guibg=NONE gui=NONE cterm=NONE
  hi Todo guifg=#35d585 guibg=NONE gui=bold cterm=bold
  "hi Comment guifg=#65858e guibg=NONE gui=NONE cterm=NONE
  "hi SpecialComment guifg=#e64547 guibg=NONE gui=NONE cterm=NONE
  "hi Todo guifg=#d24e9b guibg=NONE gui=bold cterm=bold
  hi Underlined guifg=NONE guibg=NONE gui=underline ctermfg=NONE ctermbg=NONE cterm=underline
  hi CursorIM guifg=NONE guibg=#e7e8eb gui=NONE cterm=NONE
  hi ToolbarLine guifg=NONE guibg=#353547 gui=NONE cterm=NONE
  hi ToolbarButton guifg=#64878f guibg=#353547 gui=bold cterm=bold
  hi NormalMode guifg=#949494 guibg=#e7e8eb gui=reverse cterm=reverse
  hi InsertMode guifg=#95c76f guibg=#292c36 gui=reverse cterm=reverse
  hi ReplaceMode guifg=#c98351 guibg=#292c36 gui=reverse cterm=reverse
  hi VisualMode guifg=#4670d8 guibg=#e7e8eb gui=reverse cterm=reverse
  hi CommandMode guifg=#d24e9b guibg=#e7e8eb gui=reverse cterm=reverse
  if !s:italics
    hi Folded gui=NONE cterm=NONE
    hi Comment gui=NONE cterm=NONE
  endif
  if has('nvim')
    hi! link TermCursor Cursor
    hi TermCursorNC guifg=#e7e8eb guibg=#949494 gui=NONE cterm=NONE
  endif
  unlet s:t_Co s:italics
  finish
endif

if s:t_Co >= 256
  if get(g:, 'wwdc16_transp_bg', 0)
    hi Normal ctermfg=231 ctermbg=NONE cterm=NONE
    hi CursorLineNr ctermfg=231 ctermbg=NONE cterm=NONE
    hi FoldColumn ctermfg=246 ctermbg=NONE cterm=NONE
    hi LineNr ctermfg=246 ctermbg=NONE cterm=NONE
    hi Terminal ctermfg=231 ctermbg=NONE cterm=NONE
  else
    hi Normal ctermfg=231 ctermbg=236 cterm=NONE
    if !has('patch-8.0.0616') && !has('nvim') " Fix for Vim bug
      set background=dark
    endif
    hi CursorLineNr ctermfg=231 ctermbg=237 cterm=NONE
    hi FoldColumn ctermfg=246 ctermbg=237 cterm=NONE
    hi LineNr ctermfg=246 ctermbg=237 cterm=NONE
    hi Terminal ctermfg=231 ctermbg=236 cterm=NONE
  endif
  hi ColorColumn ctermfg=NONE ctermbg=237 cterm=NONE
  hi Conceal ctermfg=66 ctermbg=NONE cterm=NONE
  hi Cursor ctermfg=231 ctermbg=26 cterm=NONE
  hi CursorColumn ctermfg=NONE ctermbg=237 cterm=NONE
  hi CursorLine ctermfg=NONE ctermbg=237 cterm=NONE
  hi DiffAdd ctermfg=150 ctermbg=236 cterm=reverse
  hi DiffChange ctermfg=173 ctermbg=231 cterm=reverse
  hi DiffDelete ctermfg=167 ctermbg=231 cterm=reverse
  hi DiffText ctermfg=71 ctermbg=231 cterm=bold,reverse
  hi Directory ctermfg=150 ctermbg=NONE cterm=NONE
  hi EndOfBuffer ctermfg=237 ctermbg=NONE cterm=NONE
  hi ErrorMsg ctermfg=167 ctermbg=231 cterm=reverse
  hi Folded ctermfg=246 ctermbg=237 cterm=italic
  hi IncSearch ctermfg=169 ctermbg=231 cterm=reverse
  hi MatchParen ctermfg=237 ctermbg=173 cterm=bold,reverse
  hi ModeMsg ctermfg=231 ctermbg=NONE cterm=NONE
  hi MoreMsg ctermfg=246 ctermbg=NONE cterm=NONE
  hi NonText ctermfg=231 ctermbg=NONE cterm=NONE
  hi Pmenu ctermfg=236 ctermbg=66 cterm=NONE
  hi PmenuSbar ctermfg=237 ctermbg=237 cterm=NONE
  hi PmenuSel ctermfg=231 ctermbg=173 cterm=NONE
  hi PmenuThumb ctermfg=237 ctermbg=173 cterm=NONE
  hi Question ctermfg=246 ctermbg=NONE cterm=NONE
  hi Search ctermfg=173 ctermbg=231 cterm=reverse
  hi SignColumn ctermfg=246 ctermbg=NONE cterm=NONE
  hi SpecialKey ctermfg=246 ctermbg=NONE cterm=NONE
  hi SpellBad ctermfg=104 ctermbg=NONE cterm=underline
  hi SpellCap ctermfg=104 ctermbg=NONE cterm=underline
  hi SpellLocal ctermfg=104 ctermbg=NONE cterm=underline
  hi SpellRare ctermfg=104 ctermbg=NONE cterm=underline
  hi StatusLine ctermfg=66 ctermbg=231 cterm=reverse
  hi StatusLineNC ctermfg=66 ctermbg=236 cterm=reverse
  hi TabLine ctermfg=236 ctermbg=66 cterm=NONE
  hi TabLineFill ctermfg=231 ctermbg=66 cterm=NONE
  hi TabLineSel ctermfg=231 ctermbg=66 cterm=NONE
  hi Title ctermfg=173 ctermbg=NONE cterm=bold
  hi VertSplit ctermfg=237 ctermbg=66 cterm=NONE
  hi Visual ctermfg=26 ctermbg=231 cterm=reverse
  hi VisualNOS ctermfg=231 ctermbg=26 cterm=NONE
  hi WarningMsg ctermfg=167 ctermbg=NONE cterm=NONE
  hi WildMenu ctermfg=231 ctermbg=169 cterm=NONE
  hi Boolean ctermfg=150 ctermbg=NONE cterm=NONE
  hi Character ctermfg=173 ctermbg=NONE cterm=NONE
  hi Comment ctermfg=246 ctermbg=NONE cterm=italic
  hi Constant ctermfg=173 ctermbg=NONE cterm=NONE
  hi Debug ctermfg=169 ctermbg=NONE cterm=NONE
  hi Delimiter ctermfg=231 ctermbg=NONE cterm=NONE
  hi Error ctermfg=167 ctermbg=231 cterm=reverse
  hi Float ctermfg=150 ctermbg=NONE cterm=NONE
  hi Function ctermfg=71 ctermbg=NONE cterm=NONE
  hi Identifier ctermfg=37 ctermbg=NONE cterm=NONE
  hi Ignore ctermfg=231 ctermbg=NONE cterm=NONE
  hi Include ctermfg=167 ctermbg=NONE cterm=NONE
  hi Keyword ctermfg=66 ctermbg=NONE cterm=NONE
  hi Label ctermfg=71 ctermbg=NONE cterm=NONE
  hi Number ctermfg=150 ctermbg=NONE cterm=NONE
  hi Operator ctermfg=37 ctermbg=NONE cterm=NONE
  hi PreProc ctermfg=169 ctermbg=NONE cterm=NONE
  hi Special ctermfg=169 ctermbg=NONE cterm=NONE
  hi SpecialChar ctermfg=169 ctermbg=NONE cterm=NONE
  hi SpecialComment ctermfg=167 ctermbg=NONE cterm=NONE
  hi Statement ctermfg=66 ctermbg=NONE cterm=NONE
  hi StorageClass ctermfg=37 ctermbg=NONE cterm=NONE
  hi String ctermfg=150 ctermbg=NONE cterm=NONE
  hi Structure ctermfg=167 ctermbg=NONE cterm=NONE
  hi Todo ctermfg=169 ctermbg=NONE cterm=bold
  hi Type ctermfg=104 ctermbg=NONE cterm=NONE
  hi Typedef ctermfg=150 ctermbg=NONE cterm=NONE
  hi Underlined ctermfg=NONE ctermbg=NONE cterm=underline
  hi CursorIM ctermfg=NONE ctermbg=231 cterm=NONE
  hi ToolbarLine ctermfg=NONE ctermbg=237 cterm=NONE
  hi ToolbarButton ctermfg=66 ctermbg=237 cterm=bold
  hi NormalMode ctermfg=246 ctermbg=231 cterm=reverse
  hi InsertMode ctermfg=150 ctermbg=236 cterm=reverse
  hi ReplaceMode ctermfg=173 ctermbg=236 cterm=reverse
  hi VisualMode ctermfg=26 ctermbg=231 cterm=reverse
  hi CommandMode ctermfg=169 ctermbg=231 cterm=reverse
  if !s:italics
    hi Folded cterm=NONE
    hi Comment cterm=NONE
  endif
  if has('nvim')
    hi! link TermCursor Cursor
    hi TermCursorNC ctermfg=231 ctermbg=246 cterm=NONE
  endif
  unlet s:t_Co s:italics
  finish
endif

if s:t_Co >= 8
  if get(g:, 'wwdc16_transp_bg', 0)
    hi Normal ctermfg=White ctermbg=NONE cterm=NONE
    hi CursorLineNr ctermfg=White ctermbg=NONE cterm=NONE
    hi FoldColumn ctermfg=Gray ctermbg=NONE cterm=NONE
    hi LineNr ctermfg=Gray ctermbg=NONE cterm=NONE
    hi Terminal ctermfg=White ctermbg=NONE cterm=NONE
  else
    hi Normal ctermfg=White ctermbg=Black cterm=NONE
    hi CursorLineNr ctermfg=White ctermbg=DarkGray cterm=NONE
    hi FoldColumn ctermfg=Gray ctermbg=DarkGray cterm=NONE
    hi LineNr ctermfg=Gray ctermbg=DarkGray cterm=NONE
    hi Terminal ctermfg=White ctermbg=Black cterm=NONE
  endif
  hi ColorColumn ctermfg=NONE ctermbg=DarkGray cterm=NONE
  hi Conceal ctermfg=DarkCyan ctermbg=NONE cterm=NONE
  hi Cursor ctermfg=White ctermbg=DarkBlue cterm=NONE
  hi CursorColumn ctermfg=NONE ctermbg=DarkGray cterm=NONE
  hi CursorLine ctermfg=NONE ctermbg=DarkGray cterm=NONE
  hi DiffAdd ctermfg=Green ctermbg=Black cterm=reverse
  hi DiffChange ctermfg=DarkYellow ctermbg=White cterm=reverse
  hi DiffDelete ctermfg=DarkRed ctermbg=White cterm=reverse
  hi DiffText ctermfg=DarkGreen ctermbg=White cterm=bold,reverse
  hi Directory ctermfg=Green ctermbg=NONE cterm=NONE
  hi EndOfBuffer ctermfg=DarkGray ctermbg=NONE cterm=NONE
  hi ErrorMsg ctermfg=DarkRed ctermbg=White cterm=reverse
  hi Folded ctermfg=Gray ctermbg=DarkGray cterm=italic
  hi IncSearch ctermfg=DarkMagenta ctermbg=White cterm=reverse
  hi MatchParen ctermfg=DarkGray ctermbg=DarkYellow cterm=bold,reverse
  hi ModeMsg ctermfg=White ctermbg=NONE cterm=NONE
  hi MoreMsg ctermfg=Gray ctermbg=NONE cterm=NONE
  hi NonText ctermfg=White ctermbg=NONE cterm=NONE
  hi Pmenu ctermfg=Black ctermbg=DarkCyan cterm=NONE
  hi PmenuSbar ctermfg=DarkGray ctermbg=DarkGray cterm=NONE
  hi PmenuSel ctermfg=White ctermbg=DarkYellow cterm=NONE
  hi PmenuThumb ctermfg=DarkGray ctermbg=DarkYellow cterm=NONE
  hi Question ctermfg=Gray ctermbg=NONE cterm=NONE
  hi Search ctermfg=DarkYellow ctermbg=White cterm=reverse
  hi SignColumn ctermfg=Gray ctermbg=NONE cterm=NONE
  hi SpecialKey ctermfg=Gray ctermbg=NONE cterm=NONE
  hi SpellBad ctermfg=Magenta ctermbg=NONE cterm=underline
  hi SpellCap ctermfg=Magenta ctermbg=NONE cterm=underline
  hi SpellLocal ctermfg=Magenta ctermbg=NONE cterm=underline
  hi SpellRare ctermfg=Magenta ctermbg=NONE cterm=underline
  hi StatusLine ctermfg=DarkCyan ctermbg=White cterm=reverse
  hi StatusLineNC ctermfg=DarkCyan ctermbg=Black cterm=reverse
  hi TabLine ctermfg=Black ctermbg=DarkCyan cterm=NONE
  hi TabLineFill ctermfg=White ctermbg=DarkCyan cterm=NONE
  hi TabLineSel ctermfg=White ctermbg=DarkCyan cterm=NONE
  hi Title ctermfg=DarkYellow ctermbg=NONE cterm=bold
  hi VertSplit ctermfg=DarkGray ctermbg=DarkCyan cterm=NONE
  hi Visual ctermfg=DarkBlue ctermbg=White cterm=reverse
  hi VisualNOS ctermfg=White ctermbg=DarkBlue cterm=NONE
  hi WarningMsg ctermfg=DarkRed ctermbg=NONE cterm=NONE
  hi WildMenu ctermfg=White ctermbg=DarkMagenta cterm=NONE
  hi Boolean ctermfg=Green ctermbg=NONE cterm=NONE
  hi Character ctermfg=DarkYellow ctermbg=NONE cterm=NONE
  hi Comment ctermfg=Gray ctermbg=NONE cterm=italic
  hi Constant ctermfg=DarkYellow ctermbg=NONE cterm=NONE
  hi Debug ctermfg=DarkMagenta ctermbg=NONE cterm=NONE
  hi Delimiter ctermfg=White ctermbg=NONE cterm=NONE
  hi Error ctermfg=DarkRed ctermbg=White cterm=reverse
  hi Float ctermfg=Green ctermbg=NONE cterm=NONE
  hi Function ctermfg=DarkGreen ctermbg=NONE cterm=NONE
  hi Identifier ctermfg=Cyan ctermbg=NONE cterm=NONE
  hi Ignore ctermfg=White ctermbg=NONE cterm=NONE
  hi Include ctermfg=DarkRed ctermbg=NONE cterm=NONE
  hi Keyword ctermfg=DarkCyan ctermbg=NONE cterm=NONE
  hi Label ctermfg=DarkGreen ctermbg=NONE cterm=NONE
  hi Number ctermfg=Green ctermbg=NONE cterm=NONE
  hi Operator ctermfg=Cyan ctermbg=NONE cterm=NONE
  hi PreProc ctermfg=DarkMagenta ctermbg=NONE cterm=NONE
  hi Special ctermfg=DarkMagenta ctermbg=NONE cterm=NONE
  hi SpecialChar ctermfg=DarkMagenta ctermbg=NONE cterm=NONE
  hi SpecialComment ctermfg=DarkRed ctermbg=NONE cterm=NONE
  hi Statement ctermfg=DarkCyan ctermbg=NONE cterm=NONE
  hi StorageClass ctermfg=Cyan ctermbg=NONE cterm=NONE
  hi String ctermfg=Green ctermbg=NONE cterm=NONE
  hi Structure ctermfg=DarkRed ctermbg=NONE cterm=NONE
  hi Todo ctermfg=DarkMagenta ctermbg=NONE cterm=bold
  hi Type ctermfg=Magenta ctermbg=NONE cterm=NONE
  hi Underlined ctermfg=NONE ctermbg=NONE cterm=underline
  hi CursorIM ctermfg=NONE ctermbg=White cterm=NONE
  hi ToolbarLine ctermfg=NONE ctermbg=DarkGray cterm=NONE
  hi ToolbarButton ctermfg=DarkCyan ctermbg=DarkGray cterm=bold
  hi NormalMode ctermfg=Gray ctermbg=White cterm=reverse
  hi InsertMode ctermfg=Green ctermbg=Black cterm=reverse
  hi ReplaceMode ctermfg=DarkYellow ctermbg=Black cterm=reverse
  hi VisualMode ctermfg=DarkBlue ctermbg=White cterm=reverse
  hi CommandMode ctermfg=DarkMagenta ctermbg=White cterm=reverse
  if !s:italics
    hi Folded cterm=NONE
    hi Comment cterm=NONE
  endif
  if has('nvim')
    hi! link TermCursor Cursor
    hi TermCursorNC ctermfg=White ctermbg=Gray cterm=NONE
  endif
  unlet s:t_Co s:italics
  finish
endif

if s:t_Co >= 2
  hi Normal term=NONE
  hi ColorColumn term=reverse
  hi Conceal term=NONE
  hi Cursor term=NONE
  hi CursorColumn term=reverse
  hi CursorLine term=underline
  hi CursorLineNr term=bold,italic,reverse,underline
  hi DiffAdd term=reverse,underline
  hi DiffChange term=reverse,underline
  hi DiffDelete term=reverse,underline
  hi DiffText term=bold,reverse,underline
  hi Directory term=NONE
  hi EndOfBuffer term=NONE
  hi ErrorMsg term=bold,italic,reverse
  hi FoldColumn term=reverse
  hi Folded term=italic,reverse,underline
  hi IncSearch term=bold,italic,reverse
  hi LineNr term=reverse
  hi MatchParen term=bold,underline
  hi ModeMsg term=NONE
  hi MoreMsg term=NONE
  hi NonText term=NONE
  hi Pmenu term=reverse
  hi PmenuSbar term=NONE
  hi PmenuSel term=NONE
  hi PmenuThumb term=NONE
  hi Question term=standout
  hi Search term=italic,underline
  hi SignColumn term=reverse
  hi SpecialKey term=bold
  hi SpellBad term=italic,underline
  hi SpellCap term=italic,underline
  hi SpellLocal term=italic,underline
  hi SpellRare term=italic,underline
  hi StatusLine term=bold,reverse
  hi StatusLineNC term=reverse
  hi TabLine term=italic,reverse,underline
  hi TabLineFill term=reverse,underline
  hi TabLineSel term=bold
  hi Title term=bold
  hi VertSplit term=reverse
  hi Visual term=reverse
  hi VisualNOS term=NONE
  hi WarningMsg term=standout
  hi WildMenu term=bold
  hi Comment term=italic
  hi Constant term=bold,italic
  hi Error term=reverse
  hi Identifier term=italic
  hi Ignore term=NONE
  hi PreProc term=italic
  hi Special term=bold,italic
  hi Statement term=bold
  hi Todo term=bold,underline
  hi Type term=bold
  hi Underlined term=underline
  hi CursorIM term=NONE
  hi ToolbarLine term=reverse
  hi ToolbarButton term=bold,reverse
  if !s:italics
    hi CursorLineNr term=bold,reverse,underline
    hi ErrorMsg term=bold,reverse
    hi Folded term=reverse,underline
    hi IncSearch term=bold,reverse
    hi Search term=underline
    hi SpellBad term=underline
    hi SpellCap term=underline
    hi SpellLocal term=underline
    hi SpellRare term=underline
    hi TabLine term=reverse,underline
    hi Comment term=NONE
    hi Constant term=bold
    hi Identifier term=NONE
    hi PreProc term=NONE
    hi Special term=bold
  endif
  unlet s:t_Co s:italics
  finish
endif

" Background: dark
" Color: black                rgb( 41,  44,  54)   236        Black
" Color: red                  rgb(230,  69,  71)   167        DarkRed
" Color: green                rgb( 66, 166,  71)   71         DarkGreen
" Color: yellow               rgb(201, 131,  81)   173        DarkYellow
" Color: blue                 rgb( 70, 112, 216)   26         DarkBlue
" Color: magenta              rgb(210,  78, 155)   169        DarkMagenta
" Color: cyan                 rgb(100, 135, 143)   66         DarkCyan
" Color: white                rgb(148, 148, 148)   246        Gray
"
" Color: brightblack          rgb( 53,  53,  71)   237        DarkGray
" Color: brightred            rgb(248,  69,  71)   203        Red
" Color: brightgreen          rgb(149, 199, 111)   150        Green
" Color: brightyellow         rgb(239, 161, 107)   216        Yellow
" Color: brightblue           rgb( 82, 131, 255)   69         Blue
" Color: brightmagenta        rgb(132, 133, 206)   104        Magenta
" Color: brightcyan           rgb(  0, 171, 165)   37         Cyan
" Color: brightwhite          rgb(255, 255, 255)   231        White
" Term Colors: black red green yellow blue magenta cyan white
" Term Colors: brightblack brightred brightgreen brightyellow
" Term Colors: brightblue brightmagenta brightcyan brightwhite
" vim: et ts=2 sw=2
